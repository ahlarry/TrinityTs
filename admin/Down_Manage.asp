...
%>
        <table width="620" border="0" cellpadding="0" cellspacing="1" bgcolor="#000000" class="border" style="word-break:break-all">
          <tr bgcolor="#A4B6D7" class="title">
            ...
            <td align="center" >所属二级分类</td>
            <td width="68" align="center" >操作</td>
          </tr>
          <%do while not rs.eof%>
          <tr class="tdbg">
            <td width="36" height="22" align="center" bgcolor="#A4B6D7"><input name='ID' type='checkbox' onclick="unselectall()" id="ID" value='<%=cstr(rs("ID"))%>' />
            </td>
            ...
            <td  bgcolor="#ECF5FF"><div align="center"><%=rs("BigClassName")%></div></td>
            <td  bgcolor="#ECF5FF"><div align="center"><%=rs("SmallClassName")%></div></td>
            <td width="68" align="center" bgcolor="#ECF5FF"><a href="Down_modi.asp?ID=<%=rs("id")%>">修改</a> <a href="Down_del.asp?ID=<%=rs("ID")%>&amp;Action=Del" onclick="return ConfirmDel();">删除</a> </td>
          </tr>
          <%
	i=i+1
        ...
        <tr class="tdbg"> 
          <form name="searchsoft" method="get" action="Down_Manage.asp">
            <td height="30"> <strong>查找下载：</strong> <input name="Title" type="text" class=smallInput id="Title" size="20" maxlength="50"> 
              <input name="Query" type2="submit" id="Query" value="查 询"> &nbsp;&nbsp;请输入下载名称。如果为空，则查找所有下载。</td>
          </form>
        </tr>
      </table></td>
  </tr>
